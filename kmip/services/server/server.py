...
class KmipServer(object):
    ...
    def start(self):
        ...
        self._ctx.set_ciphers(self.auth_suite.ciphers)
        self._ctx.load_verify_locations(cafile=self.config.settings.get('ca_path'))
        self._ctx.verify_mode = ssl.CERT_REQUIRED
#            ssl_version=self.auth_suite.protocol,

        self._socket = self._ctx.wrap_socket(
            self._socket,
            server_side=True,
